export type Language = 'en' | 'es' | 'pt' | 'de' | 'fr' | 'hi';

export const translations = {
  en: {
    common: {
      continue: 'Continue',
      back: 'Back',
      save: 'Save',
      cancel: 'Cancel',
      search: 'Search',
      loading: 'Loading...',
      error: 'Error',
      success: 'Success',
      submit: 'Submit',
      done: 'Done',
      skip: 'Skip',
      next: 'Next',
      close: 'Close',
      yes: 'Yes',
      no: 'No',
      or: 'or',
      lastUpdated: 'Last updated',
      ago: 'ago',
      minutes: 'minutes',
      hours: 'hours',
      days: 'days',
    },
    
    welcome: {
      title: 'TRUCKEREASY',
      subtitle: 'Your road companion',
      tagline: 'Find stops, report conditions, connect with drivers',
      selectLanguage: 'Select your language',
      getStarted: 'Get Started',
    },
    
    auth: {
      login: 'Log In',
      signup: 'Sign Up',
      email: 'Email',
      password: 'Password',
      confirmPassword: 'Confirm Password',
      forgotPassword: 'Forgot Password?',
      noAccount: "Don't have an account?",
      hasAccount: 'Already have an account?',
      orContinueWith: 'Or continue with',
      logout: 'Log Out',
      fullName: 'Full Name',
      phone: 'Phone',
    },
    
    onboarding: {
      welcome: 'Welcome, Driver!',
      letsSetup: "Let's set up your profile",
      truckType: 'Truck Type',
      trailerType: 'Trailer Type',
      selectTruck: 'Select your truck type',
      selectTrailer: 'Select your trailer type',
      healthProfile: 'Health & Diet Profile',
      healthSubtitle: 'Help us recommend the best meals for you',
      dietaryPreferences: 'Dietary Preferences',
      allergies: 'Allergies & Restrictions',
      healthConditions: 'Health Conditions',
      fitnessGoals: 'Fitness Goals',
      optional: '(Optional)',
    },
    
    truckTypes: {
      semi: 'Semi Truck',
      daycab: 'Day Cab',
      sleeper: 'Sleeper',
      boxtruck: 'Box Truck',
      flatbed: 'Flatbed',
    },
    
    trailerTypes: {
      dryvan: 'Dry Van',
      reefer: 'Reefer',
      flatbed: 'Flatbed',
      tanker: 'Tanker',
      container: 'Container',
      lowboy: 'Lowboy',
      none: 'No Trailer',
    },
    
    diet: {
      none: 'No restrictions',
      vegetarian: 'Vegetarian',
      vegan: 'Vegan',
      keto: 'Keto',
      lowCarb: 'Low Carb',
      glutenFree: 'Gluten Free',
      dairyFree: 'Dairy Free',
      halal: 'Halal',
      kosher: 'Kosher',
    },
    
    allergies: {
      nuts: 'Nuts',
      shellfish: 'Shellfish',
      dairy: 'Dairy',
      eggs: 'Eggs',
      soy: 'Soy',
      wheat: 'Wheat',
      fish: 'Fish',
    },
    
    health: {
      diabetes: 'Diabetes',
      hypertension: 'High Blood Pressure',
      heartDisease: 'Heart Disease',
      highCholesterol: 'High Cholesterol',
      none: 'None',
    },
    
    goals: {
      loseWeight: 'Lose Weight',
      gainMuscle: 'Gain Muscle',
      maintain: 'Maintain Weight',
      eatHealthier: 'Eat Healthier',
      moreEnergy: 'More Energy',
    },
    
    nav: {
      map: 'Map',
      stops: 'Stops',
      report: 'Report',
      community: 'Community',
      profile: 'Profile',
    },
    
    map: {
      nearMe: 'Near Me',
      searchPlaces: 'Search truck stops, restaurants...',
      filters: 'Filters',
      truckStops: 'Truck Stops',
      restaurants: 'Restaurants',
      weighStations: 'Weigh Stations',
      restAreas: 'Rest Areas',
      shippers: 'Shippers/Receivers',
      gasStations: 'Gas Stations',
    },
    
    place: {
      openNow: 'Open Now',
      closed: 'Closed',
      open24h: 'Open 24 Hours',
      unknown: 'Status Unknown',
      parking: 'Parking',
      available: 'Available',
      limited: 'Limited',
      full: 'Full',
      amenities: 'Amenities',
      reviews: 'Reviews',
      reportStatus: 'Report Status',
      directions: 'Directions',
      call: 'Call',
      photos: 'Photos',
      weighStatus: 'Weigh Station Status',
      weighOpen: 'Open',
      weighClosed: 'Closed',
      weighBypass: 'Bypass Available',
      info: 'Info',
      reports: 'Reports',
      recentReports: 'Recent Reports',
    },
    
    amenities: {
      showers: 'Showers',
      laundry: 'Laundry',
      wifi: 'WiFi',
      restaurant: 'Restaurant',
      atm: 'ATM',
      scales: 'Scales',
      repair: 'Repair Shop',
      tireService: 'Tire Service',
      truckerLounge: 'Trucker Lounge',
      pets: 'Pet Friendly',
    },
    
    report: {
      title: 'Quick Report',
      subtitle: 'Help other drivers',
      parking: 'Parking Status',
      parkingStatus: 'Parking Status',
      weighStation: 'Weigh Station',
      hazard: 'Safety Hazard',
      conditions: 'Road Conditions',
      safetyAlert: 'Safety Alert',
      roadConditions: 'Road Conditions',
      other: 'Other',
      thankYou: 'Thanks for reporting!',
      thanks: 'Thanks for reporting!',
      lotFull: 'Lot is Full',
      spotsAvailable: 'Spots Available',
      limitedSpots: 'Limited Spots',
      helpOthers: 'Help other drivers by reporting conditions',
      currentLocation: 'Current Location',
      submit: 'Submit Report',
      quickReport: 'Quick Report',
    },
    
    review: {
      title: 'Rate this Location',
      howWasIt: 'How was your experience?',
      reception: 'Reception/Service',
      organization: 'Organization',
      waitTime: 'Wait Time',
      facilities: 'Facilities',
      wouldRecommend: 'Would you recommend?',
      addComment: 'Add a comment...',
      tags: 'Add tags',
      submit: 'Submit Review',
    },

    reviews: {
      writeReview: 'Write a Review',
      shareFeedback: 'Share your experience...',
      submit: 'Submit',
      thankYou: 'Thanks for your review!',
      companyRating: 'Company Rating',
      rateShipper: 'Rate this shipper or receiver based on your experience',
      rateCompany: 'Rate Company',
      shipperReceiver: 'Shipper / Receiver',
      companyName: 'Company Name',
      enterCompanyName: 'Enter company name...',
      overallRating: 'Overall Rating',
      waitTime: 'Wait Time',
      detailedRatings: 'Detailed Ratings',
      selectTags: 'Select Tags',
      additionalNotes: 'Additional Notes',
      shareExperience: 'Share more about your experience...',
      submitReview: 'Submit Review',
      tags: {
        friendly: 'Friendly',
        organized: 'Organized',
        clean: 'Clean',
        quickLoad: 'Quick Loading',
        goodRestrooms: 'Good Restrooms',
        safeParking: 'Safe Parking',
        slowProcess: 'Slow Process',
        rude: 'Rude Staff',
        disorganized: 'Disorganized',
        noRestrooms: 'No Restrooms',
      },
      categories: {
        service: 'Service',
        organization: 'Organization',
        facilities: 'Facilities',
        loading: 'Loading/Unloading',
      },
    },
    
    reviewTags: {
      quickService: 'Quick Service',
      friendly: 'Friendly Staff',
      cleanFacilities: 'Clean',
      goodParking: 'Good Parking',
      longWait: 'Long Wait',
      disorganized: 'Disorganized',
      poorCommunication: 'Poor Communication',
      limitedParking: 'Limited Parking',
    },
    
    community: {
      title: 'Communities',
      join: 'Join',
      leave: 'Leave',
      members: 'members',
      createGroup: 'Create Group',
      groupName: 'Group Name',
      groupDescription: 'Description',
      selectRegion: 'Select Region',
      selectLanguage: 'Select Language',
      trailerType: 'Trailer Type (Optional)',
      publicGroup: 'Public Group',
      privateGroup: 'Private Group',
      all: 'All',
      byLanguage: 'By Language',
      byRegion: 'By Region',
      byTrailer: 'By Trailer',
      typeMessage: 'Type a message...',
      editNickname: 'Edit nickname',
      enableNotifications: 'Enable notifications',
      disableNotifications: 'Disable notifications',
      joinRoom: 'Join room',
      chooseNickname: 'Choose nickname and join',
      noMessages: 'No messages yet. Be the first!',
      enterRoom: 'Join the room to see messages and participate.',
      tabs: {
        news: 'News',
        learn: 'Learn',
        reports: 'Reports',
        facilities: 'Facilities',
        chat: 'Chat',
      },
    },
    
    news: {
      title: 'News',
      refresh: 'Refresh',
      refreshing: 'Refreshing...',
      updated: 'Updated',
      readMore: 'Read more',
      noNews: 'No news available',
      loadNews: 'Load news',
      errorLoading: 'Could not load news',
      tryAgain: 'Try again',
      today: 'Today',
      yesterday: 'Yesterday',
      newsUpdated: 'news updated!',
      alreadyUpdated: 'News already updated',
      urgency: {
        today: 'TODAY',
        alert: 'ALERT',
        urgent: 'URGENT',
      },
      categories: {
        weather: 'Weather',
        accident: 'Accident',
        closure: 'Closure',
        regulation: 'Regulation',
        market: 'Market',
        safety: 'Safety',
        industry: 'Industry',
      },
    },
    
    chat: {
      typeMessage: 'Type a message...',
      send: 'Send',
      online: 'online',
      members: 'members',
      report: 'Report Message',
      block: 'Block User',
    },
    
    profile: {
      title: 'Profile',
      editProfile: 'Edit Profile',
      settings: 'Settings',
      notifications: 'Notifications',
      language: 'Language',
      darkMode: 'Dark Mode',
      units: 'Units',
      imperial: 'Imperial (mi)',
      metric: 'Metric (km)',
      logout: 'Log Out',
      deleteAccount: 'Delete Account',
      myReviews: 'My Reviews',
      myReports: 'My Reports',
      stats: 'Stats',
      reportsSubmitted: 'Reports Submitted',
      reviewsWritten: 'Reviews Written',
      helpedDrivers: 'Drivers Helped',
      reports: 'Reports',
      reviews: 'Reviews',
      messages: 'Messages',
      wellbeing: 'My Wellbeing',
      myRatings: 'My Ratings',
      ratingsMade: 'ratings made',
      stopAdvisor: 'Stop Advisor',
      facilityRating: 'Facility Rating',
      foodPreferences: 'Food Preferences',
      favoriteMeals: 'Favorite Meals',
      complianceDocuments: 'Compliance & Documents',
      myDocuments: 'My Documents (CDL & Medical)',
      findDMV: 'Find DMV',
      drugTestMedical: 'Drug Test & Medical Card',
      expiringSoon: 'Expiring soon!',
      guest: 'Guest',
      signInToView: 'Sign in to view your profile',
      since: 'Since',
    },
    
    settings: {
      title: 'Settings',
      language: 'Language',
      notifications: 'Notifications',
      darkMode: 'Dark Mode',
      privacy: 'Privacy',
      help: 'Help & Support',
      contactSupport: 'Contact Support',
      roadTestMode: 'Road Test Mode',
      roadTestDesc: 'Enables diagnostics and real data',
      openTestChecklist: 'Open Test Checklist',
    },
    
    food: {
      recommendedForYou: 'Recommended for You',
      basedOnProfile: 'Based on your health profile',
      calories: 'Calories',
      protein: 'Protein',
      carbs: 'Carbs',
      healthyChoice: 'Healthy Choice',
      viewMenu: 'View Menu',
      disclaimer: 'These are suggestions based on your preferences, not medical advice.',
      convenienceSuggestions: 'Convenience Store Suggestions',
      foodSuggestions: 'Food Suggestions',
      convenienceFromStation: 'Convenience store suggestions',
      bestOption: 'Best Option',
      alternative: 'Alternative',
      ifNothingElse: 'If nothing else available',
      avoid: 'Avoid',
      nearbyRestaurants: 'Nearby restaurants:',
      tapToSeeMore: 'Tap ↑ to see more options',
      noSuggestions: 'No suggestions available at this time.',
      searchingConvenience: 'Searching convenience store options...',
      analyzingOptions: 'Analyzing options...',
      errorGettingSuggestions: 'Error getting suggestions',
    },
    
    bypass: {
      weighStation: 'Weigh Station',
      whatIsStationLike: "What's the Weigh Station like?",
      wasStationOpen: 'Was the weigh station open or closed?',
      open: 'Open',
      closed: 'Closed',
      activelyMonitored: 'Actively\nMonitored',
      tapToReport: 'Tap to report status',
      didYouReceiveBypass: 'Did you receive bypass?',
      didYouBypass: 'Did you get bypass at this station?',
      gotBypass: 'Bypass',
      pulledIn: 'Pull in',
      dontKnow: "I don't know",
      stationClosed: 'Closed',
      disclaimer: 'Driver-reported data for analysis. Not advice to avoid inspection.',
      thankYou: 'Thanks!',
      reportSaved: 'Your bypass report was saved.',
      alreadyReported: 'Already reported',
      waitMinutes: 'You already reported this station in the last 30 minutes.',
      errorSaving: 'Could not save report. Try again.',
      history: 'Bypass History',
      historyTab: 'History',
      statsTab: 'Statistics',
      noHistory: 'No bypass events yet',
      unknownStation: 'Unknown Station',
      totalEvents: 'Total Events',
      bypassRate: 'Bypass Rate',
      pullInRate: 'Pull-in Rate',
      closedRate: 'Closed Rate',
      last30Days: 'Last 30 days',
      topStates: 'Top States',
      events: 'events',
      enableReminder: 'Weigh station reminder',
      enableReminderDesc: 'Ask for bypass/pull-in after passing a station',
      saveHistory: 'Save bypass history',
      saveHistoryDesc: 'Store your bypass reports for stats',
    },

    location: {
      enableLocation: 'Enable Location',
      locationPromptDesc: 'To show nearby stops, weigh stations and restaurants, we need your location.',
      seeNearbyStops: 'See nearby stops and rest areas',
      turnByTurnNavigation: 'Accurate turn-by-turn navigation',
      weighStationAlerts: 'Weigh station alerts on your route',
      visitedStopReminder: 'Reminder to rate visited stops',
      locationPrivacy: 'Your location is not shared with other users.',
      notNow: 'Not Now',
      allow: 'Allow',
    },

    navigation: {
      title: 'Route Navigation',
      origin: 'Origin',
      destination: 'Destination',
      latitude: 'Latitude',
      longitude: 'Longitude',
      useMyLocation: 'Use my location',
      options: 'Route Options',
      truckMode: 'Truck mode',
      avoidTolls: 'Avoid tolls',
      avoidFerries: 'Avoid ferries',
      calculateRoute: 'Calculate Route',
      calculating: 'Calculating...',
      routeCalculated: 'Route calculated!',
      routeInfo: 'Route Information',
      distance: 'Distance',
      duration: 'Duration',
      mode: 'Mode',
      weatherAlerts: 'Weather Alerts',
      noAlerts: 'No weather alerts along your route',
      weatherUnavailable: 'Weather alerts unavailable in current plan',
      error: 'Error',
      fillAllFields: 'Please enter origin and destination',
      routeError: 'Failed to calculate route',
      locationObtained: 'Location obtained',
      locationError: 'Could not get current location',
      searchAddress: 'Search address...',
      noResults: 'No results found',
      startNavigation: 'Start Navigation',
      endNavigation: 'End Navigation',
      arrived: 'You have arrived!',
      continueOn: 'Continue',
    },

    subscription: {
      choosePlan: 'Choose Your Plan',
      monthly: 'Monthly',
      annual: 'Annual',
      saveUpTo: 'Save up to 17%',
      mostPopular: 'Most Popular',
      comingSoon: 'Coming Soon – Phase 2',
      truckerEasyShipper: 'Trucker Easy Shipper: Loadboard, Broker Integration & More',
      stayTuned: 'Stay Tuned',
      cancelAnytime: 'Cancel anytime. No commitment. All plans include a 7-day money-back guarantee.',
      getPlan: 'Get',
      upgradeRequired: 'Upgrade Required',
      currentPlan: 'Current Plan',
      upgradeTo: 'Upgrade to',
      manageSubscription: 'Manage Subscription',
      noSubscription: 'No Active Subscription',
      choosePlanToAccess: 'Choose a plan to access all Trucker Easy features',
      subscriptionActive: 'Subscription active',
      renews: 'Renews',
      refreshStatus: 'Refresh subscription status',
      silver: {
        name: 'Silver',
        description: 'Essential truck-aware GPS navigation',
      },
      gold: {
        name: 'Gold',
        description: 'Advanced features for professional drivers',
      },
      diamond: {
        name: 'Diamond',
        description: 'Premium experience for elite drivers',
      },
    },

    privacy: {
      privacyPolicy: 'Privacy Policy',
      policyDesc: 'Read our data handling practices',
      dataPreferences: 'Data Usage Preferences',
      preferencesDesc: 'Control how your data is used',
      requestData: 'Request Data Access or Deletion',
      requestDataDesc: 'Submit a GDPR/CCPA request',
      contactPrivacy: 'Contact Privacy Team',
      contactPrivacyDesc: 'Questions about your privacy',
      preferencesSaved: 'Preferences saved',
      requestSent: 'Request sent. Opening email...',
      messageSent: 'Message sent. Opening email...',
      lastUpdated: 'Last updated',
      agreement: 'By using Trucker Easy, you agree to this Privacy Policy.',
      sections: {
        infoCollect: 'Information We Collect',
        howWeUse: 'How We Use Your Information',
        locationData: 'Location Data Usage',
        dataSharing: 'Data Sharing & Third Parties',
        dataRetention: 'Data Retention',
        userRights: 'User Rights',
        security: 'Security Measures',
        changes: 'Changes to This Policy',
        contact: 'Contact Information',
      },
      content: {
        infoCollect: 'We collect information you provide directly (name, email, phone) and automatically (device info, location data, app usage). This includes your driver profile, truck specifications, and preferences you set within the app.',
        howWeUse: 'Your information helps us provide truck-friendly navigation, personalized food recommendations, weigh station alerts, and community features. We use data to improve our services and ensure app safety.',
        locationData: 'Location data is essential for GPS navigation, finding nearby truck stops, weigh station alerts, and community reports. Location is only collected while the app is in use for navigation purposes.',
        dataSharing: 'We do not sell your personal information. We may share data with service providers (maps, analytics) to operate the app, and with law enforcement when legally required.',
        dataRetention: 'We retain your data while your account is active. You can request deletion at any time. Anonymous analytics may be retained for service improvement.',
        userRights: 'You have the right to access, correct, delete, or export your data. California and EU residents have additional rights under CCPA and GDPR respectively.',
        security: 'We use encryption, secure servers, and access controls to protect your data. We regularly review our security practices and comply with industry standards.',
        changes: 'We may update this policy periodically. We will notify you of significant changes via the app or email.',
        contact: 'For privacy questions or to exercise your rights, contact our Privacy Team:',
      },
      prefs: {
        location: 'Location Data for Navigation',
        locationDesc: 'Required for GPS navigation (cannot be disabled)',
        analytics: 'Anonymous Usage Analytics',
        analyticsDesc: 'Help us improve the app experience',
        notifications: 'Notifications and Alerts',
        notificationsDesc: 'Weigh station, safety, and reminder alerts',
        community: 'Community Interaction Data',
        communityDesc: 'Chat messages and road reports',
      },
      form: {
        name: 'Name',
        namePlaceholder: 'Your full name',
        email: 'Email',
        emailPlaceholder: 'your@email.com',
        requestType: 'Request Type',
        requestMyData: 'Request my data',
        deleteMyData: 'Delete my data',
        message: 'Message (optional)',
        messagePlaceholder: 'Additional details...',
        sendRequest: 'Send Request',
        sendMessage: 'Send Message',
        privacyConcernPlaceholder: 'Describe your privacy concern...',
      },
    },
  },
  
  es: {
    common: {
      continue: 'Continuar',
      back: 'Atrás',
      save: 'Guardar',
      cancel: 'Cancelar',
      search: 'Buscar',
      loading: 'Cargando...',
      error: 'Error',
      success: 'Éxito',
      submit: 'Enviar',
      done: 'Listo',
      skip: 'Omitir',
      next: 'Siguiente',
      close: 'Cerrar',
      yes: 'Sí',
      no: 'No',
      or: 'o',
      lastUpdated: 'Última actualización',
      ago: 'hace',
      minutes: 'minutos',
      hours: 'horas',
      days: 'días',
    },
    
    welcome: {
      title: 'TRUCKEREASY',
      subtitle: 'Tu compañero de ruta',
      tagline: 'Encuentra paradas, reporta condiciones, conecta con conductores',
      selectLanguage: 'Selecciona tu idioma',
      getStarted: 'Comenzar',
    },
    
    auth: {
      login: 'Iniciar Sesión',
      signup: 'Registrarse',
      email: 'Correo electrónico',
      password: 'Contraseña',
      confirmPassword: 'Confirmar Contraseña',
      forgotPassword: '¿Olvidaste tu contraseña?',
      noAccount: '¿No tienes cuenta?',
      hasAccount: '¿Ya tienes cuenta?',
      orContinueWith: 'O continúa con',
      logout: 'Cerrar Sesión',
      fullName: 'Nombre Completo',
      phone: 'Teléfono',
    },
    
    onboarding: {
      welcome: '¡Bienvenido, Conductor!',
      letsSetup: 'Vamos a configurar tu perfil',
      truckType: 'Tipo de Camión',
      trailerType: 'Tipo de Remolque',
      selectTruck: 'Selecciona tu tipo de camión',
      selectTrailer: 'Selecciona tu tipo de remolque',
      healthProfile: 'Perfil de Salud y Dieta',
      healthSubtitle: 'Ayúdanos a recomendarte las mejores comidas',
      dietaryPreferences: 'Preferencias Dietéticas',
      allergies: 'Alergias y Restricciones',
      healthConditions: 'Condiciones de Salud',
      fitnessGoals: 'Objetivos de Fitness',
      optional: '(Opcional)',
    },
    
    truckTypes: {
      semi: 'Tractocamión',
      daycab: 'Cabina Día',
      sleeper: 'Con Litera',
      boxtruck: 'Camión de Caja',
      flatbed: 'Plataforma',
    },
    
    trailerTypes: {
      dryvan: 'Caja Seca',
      reefer: 'Refrigerado',
      flatbed: 'Plataforma',
      tanker: 'Tanque',
      container: 'Contenedor',
      lowboy: 'Cama Baja',
      none: 'Sin Remolque',
    },
    
    diet: {
      none: 'Sin restricciones',
      vegetarian: 'Vegetariano',
      vegan: 'Vegano',
      keto: 'Keto',
      lowCarb: 'Bajo en Carbohidratos',
      glutenFree: 'Sin Gluten',
      dairyFree: 'Sin Lácteos',
      halal: 'Halal',
      kosher: 'Kosher',
    },
    
    allergies: {
      nuts: 'Nueces',
      shellfish: 'Mariscos',
      dairy: 'Lácteos',
      eggs: 'Huevos',
      soy: 'Soya',
      wheat: 'Trigo',
      fish: 'Pescado',
    },
    
    health: {
      diabetes: 'Diabetes',
      hypertension: 'Presión Alta',
      heartDisease: 'Enfermedad Cardíaca',
      highCholesterol: 'Colesterol Alto',
      none: 'Ninguna',
    },
    
    goals: {
      loseWeight: 'Perder Peso',
      gainMuscle: 'Ganar Músculo',
      maintain: 'Mantener Peso',
      eatHealthier: 'Comer Más Saludable',
      moreEnergy: 'Más Energía',
    },
    
    nav: {
      map: 'Mapa',
      stops: 'Paradas',
      report: 'Reportar',
      community: 'Comunidad',
      profile: 'Perfil',
    },
    
    map: {
      nearMe: 'Cerca de Mí',
      searchPlaces: 'Buscar paradas, restaurantes...',
      filters: 'Filtros',
      truckStops: 'Paradas de Camiones',
      restaurants: 'Restaurantes',
      weighStations: 'Estaciones de Pesaje',
      restAreas: 'Áreas de Descanso',
      shippers: 'Embarcadores/Receptores',
      gasStations: 'Gasolineras',
    },
    
    place: {
      openNow: 'Abierto',
      closed: 'Cerrado',
      open24h: 'Abierto 24 Horas',
      unknown: 'Estado Desconocido',
      parking: 'Estacionamiento',
      available: 'Disponible',
      limited: 'Limitado',
      full: 'Lleno',
      amenities: 'Servicios',
      reviews: 'Reseñas',
      reportStatus: 'Reportar Estado',
      directions: 'Direcciones',
      call: 'Llamar',
      photos: 'Fotos',
      weighStatus: 'Estado de Pesaje',
      weighOpen: 'Abierto',
      weighClosed: 'Cerrado',
      weighBypass: 'Bypass Disponible',
      info: 'Info',
      reports: 'Reportes',
      recentReports: 'Reportes Recientes',
    },
    
    amenities: {
      showers: 'Duchas',
      laundry: 'Lavandería',
      wifi: 'WiFi',
      restaurant: 'Restaurante',
      atm: 'Cajero',
      scales: 'Básculas',
      repair: 'Taller',
      tireService: 'Servicio de Llantas',
      truckerLounge: 'Sala de Conductores',
      pets: 'Mascotas Permitidas',
    },
    
    report: {
      title: 'Reporte Rápido',
      subtitle: 'Ayuda a otros conductores',
      parking: 'Estado del Estacionamiento',
      parkingStatus: 'Estado del Estacionamiento',
      weighStation: 'Estación de Pesaje',
      hazard: 'Peligro de Seguridad',
      conditions: 'Condiciones del Camino',
      safetyAlert: 'Alerta de Seguridad',
      roadConditions: 'Condiciones del Camino',
      other: 'Otro',
      thankYou: '¡Gracias por reportar!',
      thanks: '¡Gracias por reportar!',
      lotFull: 'Estacionamiento Lleno',
      spotsAvailable: 'Lugares Disponibles',
      limitedSpots: 'Lugares Limitados',
      helpOthers: 'Ayuda a otros conductores reportando condiciones',
      currentLocation: 'Ubicación Actual',
      submit: 'Enviar Reporte',
      quickReport: 'Reporte Rápido',
    },
    
    review: {
      title: 'Califica esta Ubicación',
      howWasIt: '¿Cómo fue tu experiencia?',
      reception: 'Recepción/Servicio',
      organization: 'Organización',
      waitTime: 'Tiempo de Espera',
      facilities: 'Instalaciones',
      wouldRecommend: '¿Lo recomendarías?',
      addComment: 'Añade un comentario...',
      tags: 'Añadir etiquetas',
      submit: 'Enviar Reseña',
    },

    reviews: {
      writeReview: 'Escribir Reseña',
      shareFeedback: 'Comparte tu experiencia...',
      submit: 'Enviar',
      thankYou: '¡Gracias por tu reseña!',
      companyRating: 'Calificación de Empresa',
      rateShipper: 'Califica este embarcador o receptor según tu experiencia',
      rateCompany: 'Calificar Empresa',
      shipperReceiver: 'Embarcador / Receptor',
      companyName: 'Nombre de Empresa',
      enterCompanyName: 'Ingresa nombre de empresa...',
      overallRating: 'Calificación General',
      waitTime: 'Tiempo de Espera',
      detailedRatings: 'Calificaciones Detalladas',
      selectTags: 'Seleccionar Etiquetas',
      additionalNotes: 'Notas Adicionales',
      shareExperience: 'Comparte más sobre tu experiencia...',
      submitReview: 'Enviar Reseña',
      tags: {
        friendly: 'Amable',
        organized: 'Organizado',
        clean: 'Limpio',
        quickLoad: 'Carga Rápida',
        goodRestrooms: 'Buenos Baños',
        safeParking: 'Estacionamiento Seguro',
        slowProcess: 'Proceso Lento',
        rude: 'Personal Grosero',
        disorganized: 'Desorganizado',
        noRestrooms: 'Sin Baños',
      },
      categories: {
        service: 'Servicio',
        organization: 'Organización',
        facilities: 'Instalaciones',
        loading: 'Carga/Descarga',
      },
    },
    
    reviewTags: {
      quickService: 'Servicio Rápido',
      friendly: 'Personal Amable',
      cleanFacilities: 'Limpio',
      goodParking: 'Buen Estacionamiento',
      longWait: 'Larga Espera',
      disorganized: 'Desorganizado',
      poorCommunication: 'Mala Comunicación',
      limitedParking: 'Estacionamiento Limitado',
    },
    
    community: {
      title: 'Comunidades',
      join: 'Unirse',
      leave: 'Salir',
      members: 'miembros',
      createGroup: 'Crear Grupo',
      groupName: 'Nombre del Grupo',
      groupDescription: 'Descripción',
      selectRegion: 'Seleccionar Región',
      selectLanguage: 'Seleccionar Idioma',
      trailerType: 'Tipo de Remolque (Opcional)',
      publicGroup: 'Grupo Público',
      privateGroup: 'Grupo Privado',
      all: 'Todos',
      byLanguage: 'Por Idioma',
      byRegion: 'Por Región',
      byTrailer: 'Por Remolque',
      typeMessage: 'Escribe un mensaje...',
      editNickname: 'Editar apodo',
      enableNotifications: 'Activar notificaciones',
      disableNotifications: 'Desactivar notificaciones',
      joinRoom: 'Unirse a la sala',
      chooseNickname: 'Elegir apodo y entrar',
      noMessages: 'No hay mensajes aún. ¡Sé el primero!',
      enterRoom: 'Únete a la sala para ver mensajes y participar.',
      tabs: {
        news: 'Noticias',
        learn: 'Aprender',
        reports: 'Reportes',
        facilities: 'Instalaciones',
        chat: 'Chat',
      },
    },
    
    news: {
      title: 'Noticias',
      refresh: 'Actualizar',
      refreshing: 'Actualizando...',
      updated: 'Actualizado',
      readMore: 'Leer más',
      noNews: 'No hay noticias disponibles',
      loadNews: 'Cargar noticias',
      errorLoading: 'No se pudieron cargar las noticias',
      tryAgain: 'Reintentar',
      today: 'Hoy',
      yesterday: 'Ayer',
      newsUpdated: 'noticias actualizadas!',
      alreadyUpdated: 'Noticias ya actualizadas',
      urgency: {
        today: 'HOY',
        alert: 'ALERTA',
        urgent: 'URGENTE',
      },
      categories: {
        weather: 'Clima',
        accident: 'Accidente',
        closure: 'Cierre',
        regulation: 'Regulación',
        market: 'Mercado',
        safety: 'Seguridad',
        industry: 'Industria',
      },
    },
    
    chat: {
      typeMessage: 'Escribe un mensaje...',
      send: 'Enviar',
      online: 'en línea',
      members: 'miembros',
      report: 'Reportar Mensaje',
      block: 'Bloquear Usuario',
    },
    
    profile: {
      title: 'Perfil',
      editProfile: 'Editar Perfil',
      settings: 'Configuración',
      notifications: 'Notificaciones',
      language: 'Idioma',
      darkMode: 'Modo Oscuro',
      units: 'Unidades',
      imperial: 'Imperial (mi)',
      metric: 'Métrico (km)',
      logout: 'Cerrar Sesión',
      deleteAccount: 'Eliminar Cuenta',
      myReviews: 'Mis Reseñas',
      myReports: 'Mis Reportes',
      stats: 'Estadísticas',
      reportsSubmitted: 'Reportes Enviados',
      reviewsWritten: 'Reseñas Escritas',
      helpedDrivers: 'Conductores Ayudados',
      reports: 'Reportes',
      reviews: 'Reseñas',
      messages: 'Mensajes',
      wellbeing: 'Mi Bienestar',
      myRatings: 'Mis Calificaciones',
      ratingsMade: 'calificaciones hechas',
      stopAdvisor: 'Asesor de Paradas',
      facilityRating: 'Calificar Instalación',
      foodPreferences: 'Preferencias de Comida',
      favoriteMeals: 'Comidas Favoritas',
      complianceDocuments: 'Cumplimiento y Documentos',
      myDocuments: 'Mis Documentos (CDL y Médico)',
      findDMV: 'Encontrar DMV',
      drugTestMedical: 'Prueba de Drogas y Tarjeta Médica',
      expiringSoon: '¡Expira pronto!',
      guest: 'Invitado',
      signInToView: 'Inicia sesión para ver tu perfil',
      since: 'Desde',
    },
    
    settings: {
      title: 'Configuración',
      language: 'Idioma',
      notifications: 'Notificaciones',
      darkMode: 'Modo Oscuro',
      privacy: 'Privacidad',
      help: 'Ayuda y Soporte',
      contactSupport: 'Contactar Soporte',
      roadTestMode: 'Modo de Prueba en Carretera',
      roadTestDesc: 'Activa diagnósticos y datos reales',
      openTestChecklist: 'Abrir Lista de Verificación',
    },
    
    food: {
      recommendedForYou: 'Recomendado para Ti',
      basedOnProfile: 'Basado en tu perfil de salud',
      calories: 'Calorías',
      protein: 'Proteína',
      carbs: 'Carbohidratos',
      healthyChoice: 'Opción Saludable',
      viewMenu: 'Ver Menú',
      disclaimer: 'Estas son sugerencias basadas en tus preferencias, no consejo médico.',
      convenienceSuggestions: 'Sugerencias de Conveniencia',
      foodSuggestions: 'Sugerencias Alimentarias',
      convenienceFromStation: 'Sugerencias de la conveniencia',
      bestOption: 'Mejor Opción',
      alternative: 'Alternativa',
      ifNothingElse: 'Si no hay nada más',
      avoid: 'Evitar',
      nearbyRestaurants: 'Restaurantes cercanos:',
      tapToSeeMore: 'Toca ↑ para ver más opciones',
      noSuggestions: 'No hay sugerencias disponibles en este momento.',
      searchingConvenience: 'Buscando opciones en la conveniencia...',
      analyzingOptions: 'Analizando opciones...',
      errorGettingSuggestions: 'Error al obtener sugerencias',
    },
    
    bypass: {
      weighStation: 'Estación de Pesaje',
      whatIsStationLike: '¿Cómo está la Estación de Pesaje?',
      wasStationOpen: '¿La estación estaba abierta o cerrada?',
      open: 'Abierta',
      closed: 'Cerrada',
      activelyMonitored: 'Activamente\nMonitoreada',
      tapToReport: 'Toca para reportar estado',
      didYouReceiveBypass: '¿Recibiste bypass?',
      didYouBypass: '¿Recibiste bypass en esta estación?',
      gotBypass: 'Bypass',
      pulledIn: 'Entré',
      dontKnow: 'No sé',
      stationClosed: 'Cerrada',
      disclaimer: 'Datos reportados por conductores para análisis. No es orientación para evitar inspección.',
      thankYou: '¡Gracias!',
      reportSaved: 'Tu reporte de bypass fue guardado.',
      alreadyReported: 'Ya reportado',
      waitMinutes: 'Ya reportaste esta estación en los últimos 30 minutos.',
      errorSaving: 'No se pudo guardar el reporte. Intenta de nuevo.',
      history: 'Historial de Bypass',
      historyTab: 'Historial',
      statsTab: 'Estadísticas',
      noHistory: 'Sin eventos de bypass aún',
      unknownStation: 'Estación Desconocida',
      totalEvents: 'Total de Eventos',
      bypassRate: 'Tasa de Bypass',
      pullInRate: 'Tasa de Entrada',
      closedRate: 'Tasa Cerrada',
      last30Days: 'Últimos 30 días',
      topStates: 'Estados Top',
      events: 'eventos',
      enableReminder: 'Recordatorio de balanza',
      enableReminderDesc: 'Preguntar bypass/entrada al pasar una estación',
      saveHistory: 'Guardar historial de bypass',
      saveHistoryDesc: 'Almacenar tus reportes para estadísticas',
    },

    location: {
      enableLocation: 'Habilitar Ubicación',
      locationPromptDesc: 'Para mostrar paradas, básculas y restaurantes cercanos, necesitamos tu ubicación.',
      seeNearbyStops: 'Ver paradas y áreas de descanso cercanas',
      turnByTurnNavigation: 'Navegación precisa paso a paso',
      weighStationAlerts: 'Alertas de básculas en tu ruta',
      visitedStopReminder: 'Recordatorio para calificar paradas visitadas',
      locationPrivacy: 'Tu ubicación no se comparte con otros usuarios.',
      notNow: 'Ahora No',
      allow: 'Permitir',
    },

    navigation: {
      title: 'Navegación de Ruta',
      origin: 'Origen',
      destination: 'Destino',
      latitude: 'Latitud',
      longitude: 'Longitud',
      useMyLocation: 'Usar mi ubicación',
      options: 'Opciones de Ruta',
      truckMode: 'Modo camión',
      avoidTolls: 'Evitar peajes',
      avoidFerries: 'Evitar ferris',
      calculateRoute: 'Calcular Ruta',
      calculating: 'Calculando...',
      routeCalculated: '¡Ruta calculada!',
      routeInfo: 'Información de Ruta',
      distance: 'Distancia',
      duration: 'Duración',
      mode: 'Modo',
      weatherAlerts: 'Alertas Meteorológicas',
      noAlerts: 'Sin alertas meteorológicas en tu ruta',
      weatherUnavailable: 'Alertas meteorológicas no disponibles en el plan actual',
      error: 'Error',
      fillAllFields: 'Por favor ingrese origen y destino',
      routeError: 'Error al calcular la ruta',
      locationObtained: 'Ubicación obtenida',
      locationError: 'No se pudo obtener la ubicación actual',
      searchAddress: 'Buscar dirección...',
      noResults: 'Sin resultados',
      startNavigation: 'Iniciar Navegación',
      endNavigation: 'Terminar Navegación',
      arrived: '¡Has llegado!',
      continueOn: 'Continuar',
    },

    privacy: {
      privacyPolicy: 'Política de Privacidad',
      policyDesc: 'Lea nuestras prácticas de manejo de datos',
      dataPreferences: 'Preferencias de Uso de Datos',
      preferencesDesc: 'Controle cómo se usan sus datos',
      requestData: 'Solicitar Acceso o Eliminación',
      requestDataDesc: 'Enviar una solicitud GDPR/CCPA',
      contactPrivacy: 'Contactar Equipo de Privacidad',
      contactPrivacyDesc: 'Preguntas sobre su privacidad',
      preferencesSaved: 'Preferencias guardadas',
      requestSent: 'Solicitud enviada. Abriendo email...',
      messageSent: 'Mensaje enviado. Abriendo email...',
      lastUpdated: 'Última actualización',
      agreement: 'Al usar Trucker Easy, acepta esta Política de Privacidad.',
      sections: {
        infoCollect: 'Información que Recopilamos',
        howWeUse: 'Cómo Usamos su Información',
        locationData: 'Uso de Datos de Ubicación',
        dataSharing: 'Compartir Datos y Terceros',
        dataRetention: 'Retención de Datos',
        userRights: 'Derechos del Usuario',
        security: 'Medidas de Seguridad',
        changes: 'Cambios a esta Política',
        contact: 'Información de Contacto',
      },
      content: {
        infoCollect: 'Recopilamos información que proporciona directamente (nombre, email, teléfono) y automáticamente (info del dispositivo, ubicación, uso de la app). Esto incluye su perfil de conductor, especificaciones del camión y preferencias.',
        howWeUse: 'Su información nos ayuda a proporcionar navegación para camiones, recomendaciones de comida, alertas de estaciones de pesaje y funciones comunitarias. Usamos datos para mejorar nuestros servicios.',
        locationData: 'Los datos de ubicación son esenciales para navegación GPS, encontrar paradas cercanas, alertas de estaciones de pesaje y reportes comunitarios. La ubicación solo se recopila mientras usa la navegación.',
        dataSharing: 'No vendemos su información personal. Podemos compartir datos con proveedores de servicios (mapas, análisis) y con autoridades cuando sea legalmente requerido.',
        dataRetention: 'Retenemos sus datos mientras su cuenta esté activa. Puede solicitar eliminación en cualquier momento. Los análisis anónimos pueden conservarse para mejorar el servicio.',
        userRights: 'Tiene derecho a acceder, corregir, eliminar o exportar sus datos. Los residentes de California y la UE tienen derechos adicionales bajo CCPA y GDPR.',
        security: 'Usamos encriptación, servidores seguros y controles de acceso para proteger sus datos. Revisamos regularmente nuestras prácticas de seguridad.',
        changes: 'Podemos actualizar esta política periódicamente. Le notificaremos de cambios significativos a través de la app o email.',
        contact: 'Para preguntas de privacidad o ejercer sus derechos, contacte nuestro Equipo de Privacidad:',
      },
      prefs: {
        location: 'Datos de Ubicación para Navegación',
        locationDesc: 'Requerido para navegación GPS (no se puede desactivar)',
        analytics: 'Análisis de Uso Anónimo',
        analyticsDesc: 'Ayúdenos a mejorar la experiencia de la app',
        notifications: 'Notificaciones y Alertas',
        notificationsDesc: 'Alertas de estaciones de pesaje, seguridad y recordatorios',
        community: 'Datos de Interacción Comunitaria',
        communityDesc: 'Mensajes de chat y reportes de carretera',
      },
      form: {
        name: 'Nombre',
        namePlaceholder: 'Su nombre completo',
        email: 'Email',
        emailPlaceholder: 'su@email.com',
        requestType: 'Tipo de Solicitud',
        requestMyData: 'Solicitar mis datos',
        deleteMyData: 'Eliminar mis datos',
        message: 'Mensaje (opcional)',
        messagePlaceholder: 'Detalles adicionales...',
        sendRequest: 'Enviar Solicitud',
        sendMessage: 'Enviar Mensaje',
        privacyConcernPlaceholder: 'Describa su preocupación de privacidad...',
      },
    },
  },
  
  pt: {
    common: {
      continue: 'Continuar',
      back: 'Voltar',
      save: 'Salvar',
      cancel: 'Cancelar',
      search: 'Buscar',
      loading: 'Carregando...',
      error: 'Erro',
      success: 'Sucesso',
      submit: 'Enviar',
      done: 'Pronto',
      skip: 'Pular',
      next: 'Próximo',
      close: 'Fechar',
      yes: 'Sim',
      no: 'Não',
      or: 'ou',
      lastUpdated: 'Última atualização',
      ago: 'atrás',
      minutes: 'minutos',
      hours: 'horas',
      days: 'dias',
    },
    
    welcome: {
      title: 'TRUCKEREASY',
      subtitle: 'Seu companheiro de estrada',
      tagline: 'Encontre paradas, reporte condições, conecte-se com motoristas',
      selectLanguage: 'Selecione seu idioma',
      getStarted: 'Começar',
    },
    
    auth: {
      login: 'Entrar',
      signup: 'Cadastrar',
      email: 'E-mail',
      password: 'Senha',
      confirmPassword: 'Confirmar Senha',
      forgotPassword: 'Esqueceu a senha?',
      noAccount: 'Não tem conta?',
      hasAccount: 'Já tem conta?',
      orContinueWith: 'Ou continue com',
      logout: 'Sair',
      fullName: 'Nome Completo',
      phone: 'Telefone',
    },
    
    onboarding: {
      welcome: 'Bem-vindo, Motorista!',
      letsSetup: 'Vamos configurar seu perfil',
      truckType: 'Tipo de Caminhão',
      trailerType: 'Tipo de Carreta',
      selectTruck: 'Selecione seu tipo de caminhão',
      selectTrailer: 'Selecione seu tipo de carreta',
      healthProfile: 'Perfil de Saúde e Dieta',
      healthSubtitle: 'Nos ajude a recomendar as melhores refeições para você',
      dietaryPreferences: 'Preferências Alimentares',
      allergies: 'Alergias e Restrições',
      healthConditions: 'Condições de Saúde',
      fitnessGoals: 'Objetivos de Fitness',
      optional: '(Opcional)',
    },
    
    truckTypes: {
      semi: 'Cavalo Mecânico',
      daycab: 'Cabine Simples',
      sleeper: 'Com Leito',
      boxtruck: 'Baú',
      flatbed: 'Prancha',
    },
    
    trailerTypes: {
      dryvan: 'Baú',
      reefer: 'Refrigerado',
      flatbed: 'Prancha',
      tanker: 'Tanque',
      container: 'Contêiner',
      lowboy: 'Prancha Rebaixada',
      none: 'Sem Carreta',
    },
    
    diet: {
      none: 'Sem restrições',
      vegetarian: 'Vegetariano',
      vegan: 'Vegano',
      keto: 'Keto',
      lowCarb: 'Low Carb',
      glutenFree: 'Sem Glúten',
      dairyFree: 'Sem Lactose',
      halal: 'Halal',
      kosher: 'Kosher',
    },
    
    allergies: {
      nuts: 'Nozes',
      shellfish: 'Frutos do Mar',
      dairy: 'Laticínios',
      eggs: 'Ovos',
      soy: 'Soja',
      wheat: 'Trigo',
      fish: 'Peixe',
    },
    
    health: {
      diabetes: 'Diabetes',
      hypertension: 'Pressão Alta',
      heartDisease: 'Doença Cardíaca',
      highCholesterol: 'Colesterol Alto',
      none: 'Nenhuma',
    },
    
    goals: {
      loseWeight: 'Perder Peso',
      gainMuscle: 'Ganhar Massa',
      maintain: 'Manter Peso',
      eatHealthier: 'Comer Mais Saudável',
      moreEnergy: 'Mais Energia',
    },
    
    nav: {
      map: 'Mapa',
      stops: 'Paradas',
      report: 'Reportar',
      community: 'Comunidade',
      profile: 'Perfil',
    },
    
    map: {
      nearMe: 'Perto de Mim',
      searchPlaces: 'Buscar postos, restaurantes...',
      filters: 'Filtros',
      truckStops: 'Postos de Caminhão',
      restaurants: 'Restaurantes',
      weighStations: 'Balanças',
      restAreas: 'Áreas de Descanso',
      shippers: 'Embarcadores/Recebedores',
      gasStations: 'Postos de Combustível',
    },
    
    place: {
      openNow: 'Aberto',
      closed: 'Fechado',
      open24h: 'Aberto 24 Horas',
      unknown: 'Status Desconhecido',
      parking: 'Estacionamento',
      available: 'Disponível',
      limited: 'Limitado',
      full: 'Lotado',
      amenities: 'Comodidades',
      reviews: 'Avaliações',
      reportStatus: 'Reportar Status',
      directions: 'Direções',
      call: 'Ligar',
      photos: 'Fotos',
      weighStatus: 'Status da Balança',
      weighOpen: 'Aberta',
      weighClosed: 'Fechada',
      weighBypass: 'Bypass Disponível',
      info: 'Info',
      reports: 'Reportes',
      recentReports: 'Reportes Recentes',
    },
    
    amenities: {
      showers: 'Chuveiros',
      laundry: 'Lavanderia',
      wifi: 'WiFi',
      restaurant: 'Restaurante',
      atm: 'Caixa Eletrônico',
      scales: 'Balanças',
      repair: 'Oficina',
      tireService: 'Borracharia',
      truckerLounge: 'Sala do Motorista',
      pets: 'Pet Friendly',
    },
    
    report: {
      title: 'Reporte Rápido',
      subtitle: 'Ajude outros motoristas',
      parking: 'Status do Estacionamento',
      parkingStatus: 'Status do Estacionamento',
      weighStation: 'Balança',
      hazard: 'Perigo de Segurança',
      conditions: 'Condições da Estrada',
      safetyAlert: 'Alerta de Segurança',
      roadConditions: 'Condições da Estrada',
      other: 'Outro',
      thankYou: 'Obrigado por reportar!',
      thanks: 'Obrigado por reportar!',
      lotFull: 'Estacionamento Lotado',
      spotsAvailable: 'Vagas Disponíveis',
      limitedSpots: 'Vagas Limitadas',
      helpOthers: 'Ajude outros motoristas reportando condições',
      currentLocation: 'Localização Atual',
      submit: 'Enviar Reporte',
      quickReport: 'Reporte Rápido',
    },
    
    review: {
      title: 'Avalie este Local',
      howWasIt: 'Como foi sua experiência?',
      reception: 'Recepção/Atendimento',
      organization: 'Organização',
      waitTime: 'Tempo de Espera',
      facilities: 'Instalações',
      wouldRecommend: 'Você recomendaria?',
      addComment: 'Adicione um comentário...',
      tags: 'Adicionar tags',
      submit: 'Enviar Avaliação',
    },

    reviews: {
      writeReview: 'Escrever Avaliação',
      shareFeedback: 'Compartilhe sua experiência...',
      submit: 'Enviar',
      thankYou: 'Obrigado pela sua avaliação!',
      companyRating: 'Avaliação de Empresa',
      rateShipper: 'Avalie este embarcador ou recebedor baseado na sua experiência',
      rateCompany: 'Avaliar Empresa',
      shipperReceiver: 'Embarcador / Recebedor',
      companyName: 'Nome da Empresa',
      enterCompanyName: 'Digite nome da empresa...',
      overallRating: 'Avaliação Geral',
      waitTime: 'Tempo de Espera',
      detailedRatings: 'Avaliações Detalhadas',
      selectTags: 'Selecionar Tags',
      additionalNotes: 'Notas Adicionais',
      shareExperience: 'Compartilhe mais sobre sua experiência...',
      submitReview: 'Enviar Avaliação',
      tags: {
        friendly: 'Simpático',
        organized: 'Organizado',
        clean: 'Limpo',
        quickLoad: 'Carga Rápida',
        goodRestrooms: 'Bons Banheiros',
        safeParking: 'Estacionamento Seguro',
        slowProcess: 'Processo Lento',
        rude: 'Funcionários Grosseiros',
        disorganized: 'Desorganizado',
        noRestrooms: 'Sem Banheiros',
      },
      categories: {
        service: 'Atendimento',
        organization: 'Organização',
        facilities: 'Instalações',
        loading: 'Carga/Descarga',
      },
    },
    
    reviewTags: {
      quickService: 'Atendimento Rápido',
      friendly: 'Equipe Simpática',
      cleanFacilities: 'Limpo',
      goodParking: 'Bom Estacionamento',
      longWait: 'Longa Espera',
      disorganized: 'Desorganizado',
      poorCommunication: 'Comunicação Ruim',
      limitedParking: 'Estacionamento Limitado',
    },
    
    community: {
      title: 'Comunidades',
      join: 'Entrar',
      leave: 'Sair',
      members: 'membros',
      createGroup: 'Criar Grupo',
      groupName: 'Nome do Grupo',
      groupDescription: 'Descrição',
      selectRegion: 'Selecionar Região',
      selectLanguage: 'Selecionar Idioma',
      trailerType: 'Tipo de Carreta (Opcional)',
      publicGroup: 'Grupo Público',
      privateGroup: 'Grupo Privado',
      all: 'Todos',
      byLanguage: 'Por Idioma',
      byRegion: 'Por Região',
      byTrailer: 'Por Carreta',
      typeMessage: 'Digite uma mensagem...',
      editNickname: 'Editar apelido',
      enableNotifications: 'Ativar notificações',
      disableNotifications: 'Desativar notificações',
      joinRoom: 'Entrar na sala',
      chooseNickname: 'Escolher apelido e entrar',
      noMessages: 'Nenhuma mensagem ainda. Seja o primeiro!',
      enterRoom: 'Entre na sala para ver mensagens e participar.',
      tabs: {
        news: 'Notícias',
        learn: 'Aprender',
        reports: 'Reportes',
        facilities: 'Instalações',
        chat: 'Chat',
      },
    },
    
    news: {
      title: 'Notícias',
      refresh: 'Atualizar',
      refreshing: 'Atualizando...',
      updated: 'Atualizado',
      readMore: 'Ler mais',
      noNews: 'Nenhuma notícia disponível',
      loadNews: 'Carregar notícias',
      errorLoading: 'Não foi possível carregar as notícias',
      tryAgain: 'Tentar novamente',
      today: 'Hoje',
      yesterday: 'Ontem',
      newsUpdated: 'notícias atualizadas!',
      alreadyUpdated: 'Notícias já atualizadas',
      urgency: {
        today: 'HOJE',
        alert: 'ALERTA',
        urgent: 'URGENTE',
      },
      categories: {
        weather: 'Clima',
        accident: 'Acidente',
        closure: 'Fechamento',
        regulation: 'Regulação',
        market: 'Mercado',
        safety: 'Segurança',
        industry: 'Indústria',
      },
    },
    
    chat: {
      typeMessage: 'Digite uma mensagem...',
      send: 'Enviar',
      online: 'online',
      members: 'membros',
      report: 'Reportar Mensagem',
      block: 'Bloquear Usuário',
    },
    
    profile: {
      title: 'Perfil',
      editProfile: 'Editar Perfil',
      settings: 'Configurações',
      notifications: 'Notificações',
      language: 'Idioma',
      darkMode: 'Modo Escuro',
      units: 'Unidades',
      imperial: 'Imperial (mi)',
      metric: 'Métrico (km)',
      logout: 'Sair',
      deleteAccount: 'Excluir Conta',
      myReviews: 'Minhas Avaliações',
      myReports: 'Meus Reportes',
      stats: 'Estatísticas',
      reportsSubmitted: 'Reportes Enviados',
      reviewsWritten: 'Avaliações Escritas',
      helpedDrivers: 'Motoristas Ajudados',
      reports: 'Reportes',
      reviews: 'Avaliações',
      messages: 'Mensagens',
      wellbeing: 'Meu Bem-Estar',
      myRatings: 'Minhas Avaliações',
      ratingsMade: 'avaliações feitas',
      stopAdvisor: 'Assessor de Paradas',
      facilityRating: 'Avaliar Instalação',
      foodPreferences: 'Preferências Alimentares',
      favoriteMeals: 'Refeições Favoritas',
      complianceDocuments: 'Conformidade e Documentos',
      myDocuments: 'Meus Documentos (CDL e Médico)',
      findDMV: 'Encontrar DMV',
      drugTestMedical: 'Teste de Drogas e Cartão Médico',
      expiringSoon: 'Expirando em breve!',
      guest: 'Visitante',
      signInToView: 'Faça login para ver seu perfil',
      since: 'Desde',
    },
    
    settings: {
      title: 'Configurações',
      language: 'Idioma',
      notifications: 'Notificações',
      darkMode: 'Modo Escuro',
      privacy: 'Privacidade',
      help: 'Ajuda e Suporte',
      contactSupport: 'Contatar Suporte',
      roadTestMode: 'Modo de Teste na Estrada',
      roadTestDesc: 'Ativa diagnósticos e dados reais',
      openTestChecklist: 'Abrir Checklist de Teste',
    },
    
    food: {
      recommendedForYou: 'Recomendado para Você',
      basedOnProfile: 'Baseado no seu perfil de saúde',
      calories: 'Calorias',
      protein: 'Proteína',
      carbs: 'Carboidratos',
      healthyChoice: 'Escolha Saudável',
      viewMenu: 'Ver Cardápio',
      disclaimer: 'Estas são sugestões baseadas nas suas preferências, não orientação médica.',
      convenienceSuggestions: 'Sugestões da Conveniência',
      foodSuggestions: 'Sugestões Alimentares',
      convenienceFromStation: 'Sugestões da conveniência do posto',
      bestOption: 'Melhor Opção',
      alternative: 'Alternativa',
      ifNothingElse: 'Se não tiver nada',
      avoid: 'Evitar',
      nearbyRestaurants: 'Restaurantes próximos:',
      tapToSeeMore: 'Toque em ↑ para ver mais opções',
      noSuggestions: 'Nenhuma sugestão disponível no momento.',
      searchingConvenience: 'Buscando opções na conveniência...',
      analyzingOptions: 'Analisando opções...',
      errorGettingSuggestions: 'Erro ao obter sugestões',
    },
    
    bypass: {
      weighStation: 'Balança',
      whatIsStationLike: 'Como está a Balança?',
      wasStationOpen: 'A balança estava aberta ou fechada?',
      open: 'Aberta',
      closed: 'Fechada',
      activelyMonitored: 'Ativamente\nMonitorada',
      tapToReport: 'Toque para reportar status',
      didYouReceiveBypass: 'Você recebeu bypass?',
      didYouBypass: 'Você recebeu bypass nesta balança?',
      gotBypass: 'Bypass',
      pulledIn: 'Entrei',
      dontKnow: 'Não sei',
      stationClosed: 'Fechada',
      disclaimer: 'Dados reportados por motoristas para análise. Não é orientação para evitar fiscalização.',
      thankYou: 'Obrigado!',
      reportSaved: 'Seu reporte de bypass foi salvo.',
      alreadyReported: 'Já reportado',
      waitMinutes: 'Você já reportou esta balança nos últimos 30 minutos.',
      errorSaving: 'Não foi possível salvar o reporte. Tente novamente.',
      history: 'Histórico de Bypass',
      historyTab: 'Histórico',
      statsTab: 'Estatísticas',
      noHistory: 'Nenhum evento de bypass ainda',
      unknownStation: 'Balança Desconhecida',
      totalEvents: 'Total de Eventos',
      bypassRate: 'Taxa de Bypass',
      pullInRate: 'Taxa de Entrada',
      closedRate: 'Taxa Fechada',
      last30Days: 'Últimos 30 dias',
      topStates: 'Estados Top',
      events: 'eventos',
      enableReminder: 'Lembrete de balança',
      enableReminderDesc: 'Perguntar bypass/entrada ao passar por uma balança',
      saveHistory: 'Salvar histórico de bypass',
      saveHistoryDesc: 'Armazenar seus reportes para estatísticas',
    },

    location: {
      enableLocation: 'Ativar Localização',
      locationPromptDesc: 'Para mostrar postos, balanças e restaurantes próximos, precisamos da sua localização.',
      seeNearbyStops: 'Ver postos e paradas próximos',
      turnByTurnNavigation: 'Navegação turn-by-turn precisa',
      weighStationAlerts: 'Alertas de balanças no seu caminho',
      visitedStopReminder: 'Lembrete para avaliar postos visitados',
      locationPrivacy: 'Sua localização não é compartilhada com outros usuários.',
      notNow: 'Agora Não',
      allow: 'Permitir',
    },

    navigation: {
      title: 'Navegação de Rota',
      origin: 'Origem',
      destination: 'Destino',
      latitude: 'Latitude',
      longitude: 'Longitude',
      useMyLocation: 'Usar minha localização',
      options: 'Opções de Rota',
      truckMode: 'Modo caminhão',
      avoidTolls: 'Evitar pedágios',
      avoidFerries: 'Evitar balsas',
      calculateRoute: 'Calcular Rota',
      calculating: 'Calculando...',
      routeCalculated: 'Rota calculada!',
      routeInfo: 'Informações da Rota',
      distance: 'Distância',
      duration: 'Duração',
      mode: 'Modo',
      weatherAlerts: 'Alertas Meteorológicos',
      noAlerts: 'Sem alertas meteorológicos na sua rota',
      weatherUnavailable: 'Alertas meteorológicos indisponíveis no plano atual',
      error: 'Erro',
      fillAllFields: 'Por favor informe origem e destino',
      routeError: 'Falha ao calcular a rota',
      locationObtained: 'Localização obtida',
      locationError: 'Não foi possível obter a localização atual',
      searchAddress: 'Buscar endereço...',
      noResults: 'Nenhum resultado encontrado',
      startNavigation: 'Iniciar Navegação',
      endNavigation: 'Encerrar Navegação',
      arrived: 'Você chegou!',
      continueOn: 'Continue',
    },

    privacy: {
      privacyPolicy: 'Política de Privacidade',
      policyDesc: 'Leia nossas práticas de tratamento de dados',
      dataPreferences: 'Preferências de Uso de Dados',
      preferencesDesc: 'Controle como seus dados são usados',
      requestData: 'Solicitar Acesso ou Exclusão',
      requestDataDesc: 'Enviar solicitação GDPR/CCPA',
      contactPrivacy: 'Contatar Equipe de Privacidade',
      contactPrivacyDesc: 'Dúvidas sobre sua privacidade',
      preferencesSaved: 'Preferências salvas',
      requestSent: 'Solicitação enviada. Abrindo email...',
      messageSent: 'Mensagem enviada. Abrindo email...',
      lastUpdated: 'Última atualização',
      agreement: 'Ao usar o Trucker Easy, você concorda com esta Política de Privacidade.',
      sections: {
        infoCollect: 'Informações que Coletamos',
        howWeUse: 'Como Usamos suas Informações',
        locationData: 'Uso de Dados de Localização',
        dataSharing: 'Compartilhamento de Dados e Terceiros',
        dataRetention: 'Retenção de Dados',
        userRights: 'Direitos do Usuário',
        security: 'Medidas de Segurança',
        changes: 'Alterações nesta Política',
        contact: 'Informações de Contato',
      },
      content: {
        infoCollect: 'Coletamos informações que você fornece diretamente (nome, email, telefone) e automaticamente (info do dispositivo, localização, uso do app). Isso inclui seu perfil de motorista, especificações do caminhão e preferências.',
        howWeUse: 'Suas informações nos ajudam a fornecer navegação para caminhões, recomendações de comida, alertas de balanças e recursos da comunidade. Usamos dados para melhorar nossos serviços.',
        locationData: 'Dados de localização são essenciais para navegação GPS, encontrar paradas próximas, alertas de balanças e reportes da comunidade. A localização só é coletada enquanto você usa a navegação.',
        dataSharing: 'Não vendemos suas informações pessoais. Podemos compartilhar dados com provedores de serviços (mapas, análises) e com autoridades quando legalmente exigido.',
        dataRetention: 'Retemos seus dados enquanto sua conta estiver ativa. Você pode solicitar exclusão a qualquer momento. Análises anônimas podem ser mantidas para melhorar o serviço.',
        userRights: 'Você tem o direito de acessar, corrigir, excluir ou exportar seus dados. Residentes da Califórnia e UE têm direitos adicionais sob CCPA e GDPR.',
        security: 'Usamos criptografia, servidores seguros e controles de acesso para proteger seus dados. Revisamos regularmente nossas práticas de segurança.',
        changes: 'Podemos atualizar esta política periodicamente. Notificaremos você de alterações significativas através do app ou email.',
        contact: 'Para dúvidas de privacidade ou exercer seus direitos, contate nossa Equipe de Privacidade:',
      },
      prefs: {
        location: 'Dados de Localização para Navegação',
        locationDesc: 'Necessário para navegação GPS (não pode ser desativado)',
        analytics: 'Análises de Uso Anônimas',
        analyticsDesc: 'Ajude-nos a melhorar a experiência do app',
        notifications: 'Notificações e Alertas',
        notificationsDesc: 'Alertas de balanças, segurança e lembretes',
        community: 'Dados de Interação Comunitária',
        communityDesc: 'Mensagens de chat e reportes de estrada',
      },
      form: {
        name: 'Nome',
        namePlaceholder: 'Seu nome completo',
        email: 'Email',
        emailPlaceholder: 'seu@email.com',
        requestType: 'Tipo de Solicitação',
        requestMyData: 'Solicitar meus dados',
        deleteMyData: 'Excluir meus dados',
        message: 'Mensagem (opcional)',
        messagePlaceholder: 'Detalhes adicionais...',
        sendRequest: 'Enviar Solicitação',
        sendMessage: 'Enviar Mensagem',
        privacyConcernPlaceholder: 'Descreva sua preocupação de privacidade...',
      },
    },
  },
} as const;

export type TranslationsForLanguage = typeof translations.en;
